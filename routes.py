...
from VT.forms import LoginForm,StatusForm,UpdateForm,GrievanceForm,AddtruckForm,SearchForm
...
def status():
	...
	if form2.validate_on_submit():
		res = db.session.query(Waste).filter(Waste.waste_id==form2.truck_id.data)\
			  .filter(Waste.truck_time.like('%{}%'.format(form2.date.data) + '%'))
		return render_template('search.html',form2=form2,res=res)

	form1 = StatusForm()
	if form1.validate_on_submit():
		return render_template('track.html',form1=form1)
	
	return render_template('status.html', trucks=trucks,form1=form1,form2=form2)
...
def track():
	...
	return render_template('status.html', form1=form1,form2=form2)
